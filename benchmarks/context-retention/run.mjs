...
function pruneText(text, recoveryEligible) {
  if (!recoveryEligible) return { text, pruned: false }
...
function makeScenario() {
  ...
  for (let i = 0; i < TOOL_RESULT_COUNT; i += 1) {
    ...
    // D2B fails closed without a D1 recovery reference. The synthetic benchmark
    // therefore models all 20 oversized cases as D1-backed/recovery-eligible.
    const recoveryEligible = oversized
    ...
    results.push({ id: `tool-${i}`, oversized, recoveryEligible, text, needle })
...
const prunedResults = scenario.results.map(result => {
  const pruned = pruneText(result.text, result.recoveryEligible)
...
const report = {
  ...
  scenario: {
    ...
    recoveryEligibleOversizedToolResults: scenario.results.filter(
      result => result.oversized && result.recoveryEligible,
    ).length,
  ...
  notes: [
    ...
    'all 20 oversized synthetic results are explicitly recovery-eligible; D2B must fail closed for an oversized result without the frozen D1 recovery reference',
