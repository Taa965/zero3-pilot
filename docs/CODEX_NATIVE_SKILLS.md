...
The Skills page retains the live installation while switching modules, displays installer output, handles command/file approvals and native questions, and supports cancellation. Completion and failure refresh the catalog. A completed Codex Turn is labelled **installation task ended**, not installation success: the installer can report a repository, authentication, or existing-destination failure in a successful Turn. Users should inspect its report and the refreshed list. Installer tasks are persisted (`userData/zero3/skill-installer-jobs.json`) and their approval requests are mirrored in the Electron main process: a renderer reload re-attaches to the running install together with its still-pending approvals, and a full app restart surfaces the interrupted install in the Skills page with one-click re-install or dismiss. The approval requests themselves always die with the Codex app-server process; Zero3 recovers the install task and never fabricates an approval.
