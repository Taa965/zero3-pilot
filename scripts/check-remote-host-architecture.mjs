...
requireText(taskRunner, "['rev-parse', '--verify', '--end-of-options', `${task.target.base_ref}^{commit}`]", 'Requested base_ref must be resolved as a Git commit with option parsing terminated.')
requireText(taskRunner, 'Git base-ref preflight failed: workspace HEAD', 'Requested base_ref must match the workspace HEAD or fail closed.')
requireText(taskRunner, "['status', '--porcelain=v1', '--untracked-files=normal']", 'Clean-worktree gates must be proven from Git status through Codex.')
...
requireText(remoteConfig, 'task-mappings.json', 'Durable task mapping default path must remain an explicit host-owned artifact.')
...
console.log('Zero3 Remote Host architecture guard passed: outbound control plane -> exact task fingerprint -> durable turn intent -> narrow existing Zero3CodexAppServer Thread/Turn/read/command adapter -> pinned Codex runtime, with at-most-once Turn recovery, Codex-authoritative read-only Git preflight/postflight and no shell/Git bypass.')
