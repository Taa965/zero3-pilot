...
for (const marker of [
  ...
  'evaluateAttentionBudget', 'routeAutonomousCapabilities', 'guardEventToCandidate',
...
for (const marker of [
  ...
  'ingestGuardEvent(', 'reconcileProjectNow('
...
console.log('Zero3 Autonomous Task Runtime v1.3 guard passed: governance before task creation, existing authority reuse, lineage/budget/parent-resume, post-plugin capability gate, guard intake, planner proposals and projection-only views.')