...
const paths = {
  ...
  renderer: 'apps/zero3-desktop/renderer-v2/zero3-shell.tsx',
  taskDock: 'apps/zero3-desktop/renderer-v2/agent-task-dock.tsx',
  requestGuard: 'apps/zero3-desktop/renderer-v2/codex-background-request-guard.tsx',
  rendererApply: 'apps/zero3-desktop/scripts/apply-zero3-three-column-ui.mjs',
  gptProviderApply: 'apps/zero3-desktop/scripts/apply-gpt-web-provider.mjs',
...
requireAll(geminiProvider, paths.geminiProvider, [
  'new WebContentsView',
...
forbid(geminiProvider, paths.geminiProvider, [
  'executeJavaScript(',
  'querySelector(',
  'sendInputEvent(',
...
requireAll(antigravity, paths.antigravity, [
...
requireAll(router, paths.router, [
  'authenticated: boolean | null',
  'explicit targets are never silently changed',
  'provider runtime must verify authentication before sending the task prompt',
  'authentication is not yet proven; AUTO fallback',
...
requireAll(taskStore, paths.taskStore, [
  "createHash('sha256')",
  'agent task record identity mismatch',
...
requireAll(review, paths.review, [
  "createHash('sha256')",
  'review record identity mismatch',
...
requireAll(orchestrator, paths.orchestrator, [
  'finalizeResult: (task: Zero3TaskSpecV2, candidate: Zero3ExecutionResultV2)',
...
requireAll(recovery, paths.recovery, [
  "export type Zero3RecoveryResolution = 'KEEP_UNKNOWN' | 'ACCEPT_PARTIAL' | 'MARK_FAILED'",
  "record.state === 'OUTCOME_UNKNOWN' || record.result?.status === 'OUTCOME_UNKNOWN'",
  'recovery controller requires state and ExecutionResultV2 to both be OutcomeUnknown',
  "recoveredResult(before, 'PARTIAL'",
  "recoveredResult(before, 'FAILED'",
  'automatic retry is forbidden while OutcomeUnknown is unresolved'
])

const codexAdapter = read(paths.codexAdapter)
requireAll(codexAdapter, paths.codexAdapter, [
...
requireAll(gitAuthority, paths.git, [
  ...
  'linkedWorktree',
  'writable agent tasks require an isolated linked Git worktree',
...
requireAll(finalizer, paths.finalizer, [
  'OutcomeUnknown is unresolved; authoritative completion finalization is intentionally withheld.',
  'verifyArtifact(task, artifact)',
  'collectVerification(task, candidate)',
  "case 'git.clean'",
  "case 'artifact.hashes'",
  "case 'verification.all-passed'",
  'unknown completion gate or missing verification evidence',
  'changedFiles: git ? [...git.changedFiles] : []'
])

const verificationCollector = read(paths.verificationCollector)
requireAll(verificationCollector, paths.verificationCollector, [
  'this.executor.execCommand({',
  "sandboxPolicy: { type: 'readOnly', networkAccess: false }",
  "state: 'PASSED'",
  "state: 'FAILED'",
  "state: 'BLOCKED'",
  'verification could not be proven through Codex command/exec'
])
...
requireAll(taskPrompt, paths.taskPrompt, [
  'ZERO3_TASK_EXECUTION_ENVELOPE:',
  'Do not infer task instructions from ChatGPT or Gemini webpage DOM.',
  'commit the intended task changes and leave the task worktree clean',
  'Zero3 independently re-runs authoritative verification through Codex command/exec',
  'If a FixRequest is present, address every requiredFix in the same logical task/session'
])

const mcpCandidate = read(paths.mcpCandidate)
requireAll(mcpCandidate, paths.mcpCandidate, [
  "createHash('sha256')",
  'beginTurn(taskIdValue',
  'consumeResult(taskIdValue',
  'task MCP result candidate identity mismatch',
  'await unlinkIfPresent(file)',
  'zero3ResultCandidatesEqual'
])

const bridge = read(paths.bridge)
requireAll(bridge, paths.bridge, [
...
requireAll(artifactStore, paths.artifactStore, [
  "createHash('sha256')",
  'artifact index task identity mismatch',
  'storageName(logicalTaskId)',
  'artifact source is outside the task workspace'
])

const mcpLease = read(paths.mcpLease)
requireAll(mcpLease, paths.mcpLease, [
  "createHash('sha256')",
  'storageName(taskId)',
  'this.taskSnapshotPath = null',
  'await fs.unlink(taskSnapshotPath)',
  'Antigravity MCP lease is already installed or pending cleanup'
])

const verification = read(paths.verification)
requireAll(verification, paths.verification, [
...
if (JSON.stringify(tools) !== JSON.stringify(expectedTools)) {
  throw new Error(`${paths.taskMcp}: task-scoped MCP surface drifted: ${JSON.stringify(tools)}`)
}
requireAll(taskMcp, paths.taskMcp, [
...
requireAll(projectMcp, paths.projectMcp, [
  "createHash('sha256')",
  'storageName(id)',
  'invalid persisted project context',
  'invalid persisted handoff'
])

// Product UI authority now lives exclusively in renderer-v2. These checks are
// source-level and intentionally do not depend on the retired Hermes sidebar.
const renderer = read(paths.renderer)
requireAll(renderer, paths.renderer, [
...
forbid(renderer, paths.renderer, [
  'grep_search',
...
const taskDock = read(paths.taskDock)
requireAll(taskDock, paths.taskDock, [
...
forbid(taskDock, paths.taskDock, ['executeJavaScript(', 'querySelector(', 'sendInputEvent('])

const requestGuard = read(paths.requestGuard)
requireAll(requestGuard, paths.requestGuard, [
  'bridge.respondToServerRequest',
  'threadId === activeId',
  'code: -32004'
])

const rendererApply = read(paths.rendererApply)
requireAll(rendererApply, paths.rendererApply, [
...
const gptProviderApply = read(paths.gptProviderApply)
requireAll(gptProviderApply, paths.gptProviderApply, [
  "contextBridge.exposeInMainWorld('zero3GptWeb'",
  'applyZero3ProjectContextMcp()'
])
forbid(gptProviderApply, paths.gptProviderApply, [
  "import { applyZero3GptWebUi }",
  'applyZero3GptWebUi()'
])

const integrationApply = read(paths.integrationApply)
requireAll(integrationApply, paths.integrationApply, [
  'new Zero3RemoteTaskRunner',
  'new Zero3AgentRuntimeOrchestrator',
  'new Zero3AuthoritativeResultFinalizer',
  'new Zero3VerificationCollector',
  ...
  'zero3AgentDesktopHandlers.reviewDecision(request)'
])
...
requireAll(reviewApply, paths.reviewApply, [
  'renderZero3AgentTaskPrompt(record.task, fixRequest)',
  'renderZero3AgentTaskPrompt(task, fixRequest)',
...
requireAll(worktreeApply, paths.worktreeApply, [
...
requireAll(mcpLifecycleApply, paths.mcpLifecycleApply, [
  'new Zero3TaskMcpCandidateStore(zero3AgentTaskStateRoot)',
...
requireAll(prepare, paths.prepare, [
...
forbid(prepare, paths.prepare, [
  "import { applyZero3GptWebUi }",
  'applyZero3GptWebUi()'
])
if (!(
  prepare.indexOf('applyZero3AgentIntegrationRuntime()') < prepare.indexOf('applyZero3AgentReviewLoop()') &&
  prepare.indexOf('applyZero3AgentReviewLoop()') < prepare.indexOf('applyZero3AgentWorktreeGuard()') &&
  prepare.indexOf('applyZero3AgentWorktreeGuard()') < prepare.indexOf('applyZero3AgentMcpLifecycle()')
)) {
  throw new Error(`${paths.prepare}: final integration/review/worktree/MCP overlay order drifted`)
}

console.log('Gemini/Antigravity + Zero3 renderer-v2 architecture gate passed.')
console.log(`Checked ${Object.keys(paths).length} provider/runtime/evidence/renderer/integration boundaries from ${process.cwd()}.`)
