...
forbid(geminiProvider, paths.geminiProvider, [
  'executeJavaScript(', 'querySelector(', 'sendInputEvent(',
  'persist:zero3-chatgpt', 'chatgpt.com/backend-api', 'gemini.google.com/_/BardChatUi/'
])
...
requireAll(taskMcp, paths.taskMcp, ['cannot mutate review decisions','Zero3 CompletionGate remains authoritative','assertTask(id)','assertProject(value)'])
...
if (!(prepare.indexOf('applyZero3AgentIntegrationRuntime()') < prepare.indexOf('applyZero3AgentReviewLoop()') &&
      prepare.indexOf('applyZero3AgentReviewLoop()') < prepare.indexOf('applyZero3AgentWorktreeGuard()'))) {
  throw new Error(`${paths.prepare}: final integration/review/worktree overlay order drifted`)
}
