...
const deliveryMaterializer = read('apps/zero3-desktop/group-runtime/runtime/delivery-materializer.ts')
...
requireText(desktopRuntime, "materializeDirtyExecutorIds: ['claude']", 'explicit Claude controller-commit opt-in')
requireText(deliveryMaterializer, 'auditChangedPathOwnership', 'ownership audit before controller materialization commit')
requireText(deliveryMaterializer, 'git.stagePaths(dirtyPaths)', 'bounded staging of audited dirty paths')
requireText(deliveryMaterializer, 'git.stagedPaths()', 'post-stage ownership revalidation')
requireText(deliveryMaterializer, 'controller commit refuses pre-staged index changes', 'pre-existing Git index isolation')
requireText(deliveryMaterializer, 'git.commit(`zero3: materialize', 'controller-owned Delivery commit')
requireText(gitWorkspace, "shell: false", 'shellless Git workspace command execution')
requireText(gitWorkspace, "`:(literal)${path}`", 'literal Git pathspec staging')
requireText(gitWorkspace, 'originalPath?: string', 'rename/copy source path retention')
forbid(deliveryMaterializer, /exec(File)?\s*\(|spawn\s*\(/u, 'direct shell/process authority in Delivery materializer')
