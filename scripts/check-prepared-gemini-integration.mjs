...
function requireFile(relative) {
  if (!fs.statSync(file(relative)).isFile()) throw new Error(`prepared desktop is missing ${relative}`)
}
function requireAll(source, relative, patterns) {
  for (const pattern of patterns) {
    if (!source.includes(pattern)) throw new Error(`${relative}: prepared output is missing ${pattern}`)
  }
}
function forbid(source, relative, patterns) {
  for (const pattern of patterns) {
    if (source.includes(pattern)) throw new Error(`${relative}: prepared output still contains forbidden marker ${pattern}`)
  }
}
...
if (count(main, 'const zero3CodexAppServer = createZero3CodexAppServer()') !== 1) {
  throw new Error('electron/main.ts: Codex App Server singleton count drifted')
}
...
if (!(oldReview >= 0 && oldReview < removeReview && removeReview < newReview)) {
  throw new Error('electron/main.ts: legacy review decision handler is not safely replaced by authoritative task-state handler')
}
...
requireAll(handoff, handoffPath, [
...
forbid(handoff, handoffPath, [
  "completionGate: target === 'CODEX' ?",
  "['result.json.valid','scope.valid','review.packet.generated']"
])

const geminiUiPath = 'src/app/chat/sidebar/gemini-session-section.tsx'
const geminiUi = read(geminiUiPath)
requireAll(geminiUi, geminiUiPath, [
  'CustomEvent<{ entryId: string; taskId?: string }>',
