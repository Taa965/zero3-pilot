//! Deterministic, model-free pruning for historical tool results.
//!
//! D2A owns the pure pruning engine below. D2B adapts that engine to Codex
//! compaction through a typed extension hook and validates recoverability using
//! D1's frozen SpillStore contract. D2 never defines a competing SpillRef or
//! mutates authoritative session history.

mod extension;
mod projection;

pub use extension::ContextRetentionExtension;
...
pub struct ToolResultEnvelope<Rich, RecoveryRef> {
    ...
    /// Opaque to the pruning engine. D2B supplies a D1-verified recovery locator.
...
pub struct PruneProvenance {
    ...
    /// Unicode scalar values in the original text blocks.
    pub original_chars: usize,
    /// Unicode scalar values in the emitted model projection, including the marker.
    pub retained_chars: usize,
    /// Original Unicode scalar values excised from the logical middle span.
    pub removed_chars: usize,
    /// Net reduction after accounting for the inserted marker.
    pub net_chars_saved: usize,
...
pub fn prune_content<Rich: Clone>(
    ...
    let removed_chars = removed_end - removed_start;
    ...
    let net_chars_saved = total_chars - retained_chars;
...
/// Prunes a recoverable tool-result envelope without changing call pairing or
/// the caller's opaque recovery reference. Missing recovery evidence fails
/// closed and leaves the model projection untouched.
pub fn prune_tool_result<Rich: Clone, RecoveryRef: Clone>(
    ...
    if result.recovery_ref.is_none() {
        return Ok(None);
    }
