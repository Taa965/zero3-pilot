...
try {
  ...
  await page.screenshot({path:path.join(output,'task-context-menu.png'),fullPage:true})
  ...
  await page.getByRole('dialog',{name:'删除任务确认'}).waitFor()
  await page.screenshot({path:path.join(output,'task-delete-confirm.png'),fullPage:true})
