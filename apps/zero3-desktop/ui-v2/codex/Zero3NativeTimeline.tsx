...
export function Zero3NativeTimeline({ events }: { events: Zero3SessionEvent[] }) {
  ...
  return (
    <div className="space-y-4">
      {events.map(event => {
        if (event.type === 'userMessage' || event.type === 'agentMessage') return <Message key={event.eventId} event={event} />
        if (event.type === 'reasoning') return <Reasoning key={event.eventId} event={event} />
        if (event.type === 'commandExecution') return <Command key={event.eventId} event={event} />
        if (event.type === 'fileChange') return <FileChange key={event.eventId} event={event} />
        if (event.type === 'mcpToolCall') return <Tool key={event.eventId} event={event} />
        if (event.type === 'providerSwitch') return <ProviderSwitch key={event.eventId} event={event} />
        if (event.type === 'turnState' && event.payload.status === 'failed') {
          return <div key={event.eventId} className="max-w-[92%] rounded border border-red-500/30 bg-red-500/5 px-3 py-2 text-xs text-red-600">Turn 执行失败：{string(event.payload.error) || 'unknown error'}</div>
        }
        return null
      })}
    </div>
