...
function emptySession(): StoredSession {
  return { version: 1, nextSeq: 1, events: [], binding: emptyBinding(), coverage: { coveredSessionSeq: 0, coveredRanges: [] }, pendingHandoff: null }
}
...
export const Zero3SessionEventStore = {
  ...
  markCoveredThrough(logicalSessionId: string, sessionSeq: number) {
    ...
    mutate(logicalSessionId, session => ({ ...session, coverage: { ...session.coverage, coveredSessionSeq: Math.max(session.coverage.coveredSessionSeq, sessionSeq) } }))
  },
  ...
  markCoveredRange(logicalSessionId: string, start: number, end: number) {
    ...
    mutate(logicalSessionId, session => ({ ...session, coverage: { ...session.coverage, coveredRanges: [...session.coverage.coveredRanges, [start, end] as [number, number]] } }))
  },
  ...
  buildProviderHandoff(logicalSessionId: string, input: {
    ...
    const session = this.snapshot(logicalSessionId)
    const uncoveredEvents = session.events.filter(event => !covered(event.sessionSeq, session.coverage))
  ...
  stageProviderSwitch(logicalSessionId: string, handoff: Zero3ProviderHandoff, binding: {
    ...
    mutate(logicalSessionId, session => ({
      ...
      binding: {
        ...session.binding,
        generation: handoff.handoff.target_runtime_generation,
    ...
    append(logicalSessionId, 'providerSwitch', {
      phase: 'complete', from: handoff.from, to: handoff.to,
      inheritedEventCount: handoff.uncovered_session_delta.events.length,
      sharedMemoryStatus: handoff.shared_memory.status ?? 'unknown'
    }, { profileId: binding.profileId })
