...
export const McpHttpAdapter = {
  ...
  async status(): Promise<Zero3McpHttpStatusRecord> {
    return requireBridge().status()
  },
  async setProjectAccess(projectId: string, enabled: boolean) {
    return requireBridge().setProjectAccess({ projectId, enabled })
  },
  async rotateToken(): Promise<string> {
    return (await requireBridge().rotateToken()).bearerToken
  }
}
