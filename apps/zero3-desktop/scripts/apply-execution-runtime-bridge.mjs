...
const preloadBridge = String.raw`contextBridge.exposeInMainWorld('zero3Execution', {
  ...
  setTaskArchived: (taskId, archived) => ipcRenderer.invoke('zero3:execution:set-task-archived', taskId, archived),
  deleteTask: taskId => ipcRenderer.invoke('zero3:execution:delete-task', taskId),
  ...
  recordArtifact: (taskId, stepId, artifact) => ipcRenderer.invoke('zero3:execution:record-artifact', taskId, stepId, artifact),
...
const globalBridgeProperty = String.raw`    zero3Execution: {
      ...
      deleteTask: (taskId: string) => Promise<{ taskId: string; deleted: true }>
