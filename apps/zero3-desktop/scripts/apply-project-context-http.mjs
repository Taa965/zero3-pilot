...
function zero3McpHttpStateDir() {
  return path.join(app.getPath('userData'), 'zero3')
}
function zero3McpHttpTokenFile() {
  return path.join(zero3McpHttpStateDir(), 'mcp-http-token')
}
function zero3McpHttpPolicyFile() {
  return path.join(zero3McpHttpStateDir(), 'mcp-http-access.json')
}
...
function zero3StartProjectContextHttp() {
  ...
  zero3ProjectContextHttpChild = spawn(process.execPath, [serverPath], {
    env: {
      ...
      ZERO3_PROJECT_CONTEXT_DIR: path.join(app.getPath('userData'), 'zero3', 'project-context'),
      ZERO3_MCP_HTTP_STATE_DIR: zero3McpHttpStateDir(),
      ZERO3_MCP_HTTP_HOST: '127.0.0.1',
      ZERO3_MCP_HTTP_PORT: String(ZERO3_MCP_HTTP_PORT)
    },
...
function zero3StopProjectContextHttp() {
  zero3ProjectContextHttpChild?.kill()
  zero3ProjectContextHttpChild = null
}
...
export function applyZero3ProjectContextHttp() {
  ...
  patchFile('electron/main.ts', [
    {
      label: 'project-context HTTP runtime before Codex singleton',
      appliedMarker: 'function zero3StartProjectContextHttp() {',
      from: 'const zero3CodexAppServer = createZero3CodexAppServer()',
      to: mainRuntime + '\nconst zero3CodexAppServer = createZero3CodexAppServer()'
    },
    {
      label: 'start project-context HTTP server on Electron ready',
      appliedMarker: '  zero3StartProjectContextHttp()\n',
      from: "app.whenReady().then(() => {",
      to: "app.whenReady().then(() => {\n  zero3StartProjectContextHttp()"
    }
  ])
  patchFile('electron/preload.ts', [
    {
      label: 'project-context HTTP preload surface',
      appliedMarker: "contextBridge.exposeInMainWorld('zero3McpHttp'",
  ...
  patchFile('src/global.d.ts', [
    {
      label: 'project-context HTTP renderer type definitions',
      appliedMarker: 'type Zero3McpHttpStatus = {',
      from: 'type Zero3GptWebWorkspaceEntry = {',
      to: globalTypes + '\ntype Zero3GptWebWorkspaceEntry = {'
    },
    {
      label: 'project-context HTTP renderer surface',
      appliedMarker: '    zero3McpHttp: {',
      from: '    zero3Workspace: {',
      to: globalSurface
    }
  ])
