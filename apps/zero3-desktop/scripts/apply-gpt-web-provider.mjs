...
type Zero3GptWebEvent =
  ...
  | { kind: 'execution'; entryId: string; executing: boolean; health: 'active' | 'idle' | 'stalled' | 'timeout_error' | 'recovering' | 'recovery_failed' | null; lastProgressAt: number | null; idleForMs: number; recoveryAttempt: 0 | 1 }
`
...
const globalWindowSurface = String.raw`    zero3GptWeb: {
      ...
      executionStatus: (request: { id: string }) => Promise<{ executing: boolean; health: 'active' | 'idle' | 'stalled' | 'timeout_error' | 'recovering' | 'recovery_failed' | null; lastProgressAt: number | null; idleForMs: number; recoveryAttempt: 0 | 1 }>
...
export function applyZero3GptWebProvider() {
  ...
  patchFile('src/global.d.ts', [{
    ...
    already: "health: 'active' | 'idle' | 'stalled' | 'timeout_error' | 'recovering' | 'recovery_failed' | null",
    from: "  | { kind: 'execution'; entryId: string; executing: boolean }",
    to: "  | { kind: 'execution'; entryId: string; executing: boolean; health: 'active' | 'idle' | 'stalled' | 'timeout_error' | 'recovering' | 'recovery_failed' | null; lastProgressAt: number | null; idleForMs: number; recoveryAttempt: 0 | 1 }"
  }])
  patchFile('src/global.d.ts', [{
    ...
    to: "      executionStatus: (request: { id: string }) => Promise<{ executing: boolean; health: 'active' | 'idle' | 'stalled' | 'timeout_error' | 'recovering' | 'recovery_failed' | null; lastProgressAt: number | null; idleForMs: number; recoveryAttempt: 0 | 1 }>"
  }])
