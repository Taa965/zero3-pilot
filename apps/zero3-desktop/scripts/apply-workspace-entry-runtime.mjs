...
function patchFile(relativePath, replacements) {
  ...
  for (const replacement of replacements) {
    ...
    if (!source.includes(replacement.from)) {
      throw new Error(
        `Zero3 workspace-entry overlay drift in ${relativePath}: could not find ${replacement.label}. ` +
...
function zero3WorkspaceRequestId(value: unknown): string {
  ...
  if (typeof id !== 'string' || !id.trim() || id.trim().length > 256) {
    throw new Error('workspace entry id is required and must be at most 256 characters')
  }
  return id.trim()
}

ipcMain.handle('zero3:workspace:list', () => zero3WorkspaceEntries.list())
ipcMain.handle('zero3:workspace:get', (_event, request: unknown) =>
  zero3WorkspaceEntries.get(zero3WorkspaceRequestId(request))
)
ipcMain.handle('zero3:workspace:gpt-web:create', (_event, request: unknown) => {
  ...
  return zero3WorkspaceEntries.createGptWeb({ projectId: projectId as string | null })
})
...
ipcMain.handle('zero3:workspace:remove', (_event, request: unknown) =>
  zero3WorkspaceEntries.remove(zero3WorkspaceRequestId(request))
)
`
...
const globalWindowSurface = String.raw`    zero3Workspace: {
      ...
      createGptWeb: (request?: { projectId?: string | null }) => Promise<Zero3WorkspaceEntry>
...
export function applyZero3WorkspaceEntryRuntime() {
  ...
  patchFile('electron/main.ts', [
    {
      label: 'workspace runtime import before user-data override',
    ...
    {
      label: 'workspace registry before Codex app-server singleton',
  ...
  patchFile('electron/preload.ts', [
    {
      label: 'workspace preload surface before Codex surface',
      from: "contextBridge.exposeInMainWorld('zero3Codex', {",
      to: preloadBridge
    }
  ])
  ...
  patchFile('src/global.d.ts', [
    {
      label: 'workspace renderer type definitions',
      from: 'type Zero3CodexStatus = {',
      to: globalTypeDefinitions + '\ntype Zero3CodexStatus = {'
    },
    {
      label: 'workspace renderer window surface',
      from: '    zero3Codex: {',
