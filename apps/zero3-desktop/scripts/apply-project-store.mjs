...
function copyProjectRuntimeSources() {
  ...
  for (const line of ["export * from './project-store'", "export * from './project-types'"]) {
...
const zero3Projects = new Zero3ProjectStore(
  path.join(app.getPath('userData'), 'zero3', 'projects.json')
)

function zero3ProjectRecord(value: unknown): Record<string, unknown> {
  return value && typeof value === 'object' && !Array.isArray(value) ? (value as Record<string, unknown>) : {}
}
function zero3ProjectRequestId(value: unknown): string {
  const id = zero3ProjectRecord(value).id
  if (typeof id !== 'string' || !id.trim() || id.trim().length > 256) throw new Error('project id is required and must be at most 256 characters')
  return id.trim()
}
...
ipcMain.handle('zero3:project:create', (_event, request: unknown) => {
  const input = zero3ProjectRecord(request)
  if (typeof input.name !== 'string') throw new Error('project name is required')
  if (typeof input.rootPath !== 'string') throw new Error('project rootPath is required')
  return zero3Projects.create({ name: input.name, rootPath: input.rootPath })
})
ipcMain.handle('zero3:project:update', (_event, request: unknown) => {
  const input = zero3ProjectRecord(request)
  const id = zero3ProjectRequestId(input)
  if (input.name !== undefined && typeof input.name !== 'string') throw new Error('project name must be a string')
  if (input.chatGptProjectUrl !== undefined && input.chatGptProjectUrl !== null && typeof input.chatGptProjectUrl !== 'string') {
    throw new Error('project chatGptProjectUrl must be a string or null')
  }
  return zero3Projects.update({
    id,
    ...(input.name !== undefined ? { name: input.name as string } : {}),
    ...(input.chatGptProjectUrl !== undefined ? { chatGptProjectUrl: input.chatGptProjectUrl as string | null } : {})
  })
})
ipcMain.handle('zero3:project:remove', (_event, request: unknown) => zero3Projects.remove(zero3ProjectRequestId(request)))
ipcMain.handle('zero3:project:pickDirectory', async () => {
...
const preloadBridge = String.raw`contextBridge.exposeInMainWorld('zero3Project', {
  list: () => ipcRenderer.invoke('zero3:project:list'),
  get: request => ipcRenderer.invoke('zero3:project:get', request),
  create: request => ipcRenderer.invoke('zero3:project:create', request),
  update: request => ipcRenderer.invoke('zero3:project:update', request),
  remove: request => ipcRenderer.invoke('zero3:project:remove', request),
...
export function applyZero3ProjectStore() {
  ...
  patchFile('electron/main.ts', [
    {
      ...
      to: "import { Zero3ProjectStore, Zero3WorkspaceEntryStore } from './zero3/workspace/index'"
    },
    {
      label: 'project handlers before Codex app-server singleton',
      appliedMarker: "ipcMain.handle('zero3:project:list'",
  ...
  patchFile('electron/preload.ts', [
    {
      label: 'project preload surface before workspace surface',
      appliedMarker: "contextBridge.exposeInMainWorld('zero3Project'",
  ...
  patchFile('src/global.d.ts', [
    {
      label: 'project renderer type definitions',
      appliedMarker: 'type Zero3Project = {',
      from: 'type Zero3GptWebWorkspaceEntry = {',
      to: globalTypeDefinitions + '\ntype Zero3GptWebWorkspaceEntry = {'
    },
    {
      label: 'project renderer window surface',
      appliedMarker: '    zero3Project: {',
      from: '    zero3Workspace: {',
