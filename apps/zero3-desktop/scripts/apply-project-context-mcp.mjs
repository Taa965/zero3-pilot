...
function patchFile(relativePath, replacements) {
  ...
  for (const replacement of replacements) {
    ...
    if (!content.includes(replacement.from)) throw new Error(`Zero3 project-context MCP overlay drift in ${relativePath}: missing ${replacement.label}`)
...
function zero3WithProjectContextMcp(method: string, params: unknown): unknown {
  ...
  const projectId = typeof input.zero3ProjectId === 'string' && input.zero3ProjectId.trim() ? input.zero3ProjectId.trim() : undefined
  const cleanInput = { ...input }; delete cleanInput.zero3ProjectId
  const existing = cleanInput.config && typeof cleanInput.config === 'object' && !Array.isArray(cleanInput.config) ? cleanInput.config as Record<string, unknown> : {}
  return { ...cleanInput, config: { ...existing, ...zero3ProjectContextMcpConfig(projectId) } }
}
`
export function applyZero3ProjectContextMcp() {
  ...
  patchFile('electron/main.ts', [
    ...
    { label: 'active project parsing in Codex thread/start', appliedMarker: "const projectId = zero3CodexOptionalString(input.projectId, 'projectId', 256)", from: "  const ephemeral = zero3CodexOptionalBoolean(input.ephemeral, 'ephemeral')", to: "  const ephemeral = zero3CodexOptionalBoolean(input.ephemeral, 'ephemeral')\n  const projectId = zero3CodexOptionalString(input.projectId, 'projectId', 256)\n  if (projectId && !/^[A-Za-z0-9._:-]+$/.test(projectId)) throw new Error('projectId contains unsupported characters')" },
    { label: 'active project transfer into local-only Codex params', appliedMarker: '  if (projectId) params.zero3ProjectId = projectId', from: "  if (ephemeral != null) params.ephemeral = ephemeral\n  return params", to: "  if (ephemeral != null) params.ephemeral = ephemeral\n  if (projectId) params.zero3ProjectId = projectId\n  return params" }
  ])
  patchFile('src/global.d.ts', [{ label: 'projectId on Codex thread/start renderer request', appliedMarker: '  projectId?: string\n  sandbox?: Zero3CodexSandbox', from: '  modelProvider?: string\n  sandbox?: Zero3CodexSandbox', to: '  modelProvider?: string\n  projectId?: string\n  sandbox?: Zero3CodexSandbox' }])
  const primaryChat = path.join(hermesDesktopDir, 'src', 'app', 'zero3-codex', 'primary-chat.ts')
  if (fs.existsSync(primaryChat)) patchFile('src/app/zero3-codex/primary-chat.ts', [{
    label: 'active project selection before Codex thread/start', appliedMarker: "window.localStorage.getItem('zero3.active-project-id')",
