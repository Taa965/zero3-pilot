...
export async function readAccessPolicy(options = {}) {
  ...
  try {
    ...
    if (parsed?.schemaVersion !== POLICY_SCHEMA_VERSION || !parsed.projects || typeof parsed.projects !== 'object' || Array.isArray(parsed.projects)) {
      throw new Error('invalid MCP HTTP access policy')
    }
    const projects = {}
...
export function filterWebEgressPayload(payload) {
  ...
  const input = payload
  const output = {}
  if (Array.isArray(input.decisions)) output.decisions = input.decisions
  if (Array.isArray(input.pitfalls)) output.pitfalls = input.pitfalls
  if (input.glossary && typeof input.glossary === 'object' && !Array.isArray(input.glossary)) output.glossary = input.glossary
  return output
}
...
export function mergeWebIngressPayload(currentPayload, webPayload) {
  ...
  if ('decisions' in webPayload) {
    if (!Array.isArray(webPayload.decisions)) throw new Error('decisions must be an array')
    current.decisions = webPayload.decisions
  }
  if ('pitfalls' in webPayload) {
    if (!Array.isArray(webPayload.pitfalls)) throw new Error('pitfalls must be an array')
    current.pitfalls = webPayload.pitfalls
  }
