...
function serverFactory(core) {
  ...
  server.registerTool('project_put_context', {
    title: 'Update Zero3 Project Context', description: 'Replace canonical project context with optimistic version control. Re-read and merge after a version conflict; never overwrite it blindly.',
    inputSchema: z.object({ projectId: ID, expectedVersion: z.number().int().nonnegative().optional(), payload: z.unknown() }),
  ...
  server.registerTool('handoff_publish', {
    ...
    inputSchema: z.object({ taskId: ID, expectedVersion: z.number().int().nonnegative().optional(), result: z.record(z.string(), z.unknown()) }),
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false }
