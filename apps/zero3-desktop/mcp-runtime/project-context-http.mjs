...
const CONTEXT_DIR = process.env.ZERO3_PROJECT_CONTEXT_DIR
const core = createProjectContextCore({ rootDir: CONTEXT_DIR })
...
async function allowedProject(projectId, tool) {
  const allowed = await isProjectWebAllowed(projectId, { stateDir: STATE_DIR })
  if (!allowed) {
    ...
    server.registerTool(
      ...
      async ({ projectId, expectedVersion, payload }) => {
        ...
        try {
          ...
          const merged = mergeWebIngressPayload(current.payload, payload)
          const next = await core.putProject(projectId, expectedVersion, merged)
...
function configuredHosts() {
  const values = (process.env.ZERO3_MCP_HTTP_ALLOWED_HOSTS ?? '')
    .split(',')
...
const extraOrigins = new Set(
  (process.env.ZERO3_MCP_HTTP_ALLOWED_ORIGINS ?? '')
    .split(',')
