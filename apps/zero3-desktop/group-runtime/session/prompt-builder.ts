...
export function buildDevelopmentSessionPrompt(input: {
  ...
  return [
    ...
    '- Before final Delivery, leave the Session worktree clean; commit every intended Session change when the bound executor has Git metadata permission.',
    '- If an external headless executor cannot write Git metadata, do not bypass its permission boundary; leave only intended owned-path edits in the worktree so Zero3 can ownership-audit and materialize the Delivery commit.',
    '- If the intended changes cannot be committed or safely materialized, do not report success; surface the blocker instead.',
