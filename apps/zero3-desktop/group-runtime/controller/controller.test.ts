...
import type { DevelopmentGroupDefinition, DevelopmentGroupRuntimeState, DevelopmentSessionDefinition } from '../contracts/index.ts'
...
test('monitor makes OutcomeUnknown and ledger lag blocking rather than retryable', () => {
  const signals = observeDevelopmentGroup({ definition, state, sessions: [session], observations: [{ runtime: { groupId: 'G1', sessionId: 'S1', executionId: 'E1', status: 'outcome_unknown', attempt: 1, writerGeneration: 1, lastEventSequence: 0, updatedAt: state.updatedAt } }], events: [{ eventId: '1', sequence: 1, at: state.updatedAt, groupId: 'G1', type: 'group.created' }] })
