...
class MismatchedFailureSourceExecutor extends FakeExecutor {
  async *prompt(session: ExecutorSession, input: ExecutorInput): AsyncIterable<ExecutorEvent> {
    this.prompts.push({ session, input })
    yield {
      type: 'failure',
      sequence: 1,
      at: '2026-08-30T00:00:00.000Z',
      failure: createExecutorFailure('quota_exhausted', 'quota', 'other-executor')
    }
  }
}
...
test('frozen failure policy preserves safety and future R4F routing semantics', () => {
  for (const code of ['permission_denied', 'policy_denied', 'budget_exhausted', 'bad_request', 'user_stopped'] as const) {
    ...
    assert.equal(failurePolicyFor(code).retryable, false)
  }

  assert.deepEqual(failurePolicyFor('quota_exhausted'), { retryable: false, failover: 'eligible' })
  assert.deepEqual(failurePolicyFor('rate_limited'), { retryable: true, failover: 'conditional' })
  assert.deepEqual(failurePolicyFor('provider_overloaded'), { retryable: true, failover: 'conditional' })
  assert.deepEqual(failurePolicyFor('provider_error'), { retryable: true, failover: 'conditional' })
  assert.deepEqual(failurePolicyFor('unsupported'), { retryable: false, failover: 'conditional' })
})

test('runtime failure validation accepts frozen codes and rejects provider-defined unknown codes', () => {
  assert.equal(
  ...
  assert.equal(isExecutorFailure(createExecutorFailure('provider_error', 'provider unavailable', 'api')), true)
  assert.equal(isExecutorFailure(createExecutorFailure('user_stopped', 'cancelled by user', 'executor-core')), true)
})
...
test('manager rejects a provider failure event that spoofs another failure source', async () => {
  const registry = new Zero3ExecutorRegistry()
  registry.register(new MismatchedFailureSourceExecutor('bound-executor'))
  const manager = new Zero3ExecutorManager(registry)
  await manager.start('bound-executor', identity, policy)

  await assert.rejects(async () => {
    for await (const _event of manager.prompt(identity, {
      kind: 'prompt',
      clientRequestId: 'request-source-spoof',
      text: 'continue'
    })) {
      // Drain the stream; the manager must reject before exposing spoofed provenance.
    }
  }, /failure source does not match the bound executor/)
})
...
test('router derives failover safety from the failure code rather than provider metadata', () => {
  ...
  assert.deepEqual(
    router.fallbackCandidatesAfter('native', createExecutorFailure('user_stopped', 'cancelled', 'executor-core')),
    []
  )
  assert.throws(
    () =>
