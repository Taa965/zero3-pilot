...
pub trait MemoryRepository: Send + Sync + 'static {
    ...
    async fn events_after(&self, sequence: i64, projects: &[String]) -> anyhow::Result<Vec<CommittedEvent>>;
...
impl MemoryRepository for InMemoryRepository {
    async fn append(&self, event: MemoryEvent) -> anyhow::Result<AppendOutcome> {
        ...
        if let Some(sequence) = state.by_id.get(&event.event_id).copied() {
            return Ok(AppendOutcome { event_id: event.event_id, status: AppendStatus::Duplicate, sequence });
        }
        state.sequence += 1;
        let sequence = state.sequence;
        state.by_id.insert(event.event_id.clone(), sequence);
        state.events.push(CommittedEvent { sequence, event: event.clone() });
        Ok(AppendOutcome { event_id: event.event_id, status: AppendStatus::Accepted, sequence })
    }

    async fn events_after(&self, sequence: i64, projects: &[String]) -> anyhow::Result<Vec<CommittedEvent>> {
        let allow: HashSet<&str> = projects.iter().map(String::as_str).collect();
        Ok(self.state.lock().await.events.iter()
            .filter(|item| item.sequence > sequence)
            .filter(|item| item.event.scope.project_id.as_deref().is_some_and(|id| allow.contains(id)))
            .cloned()
            .collect())
    }
    ...
    async fn project_context(&self, project_id: &str) -> anyhow::Result<ProjectContext> {
        ...
        for item in state.events.iter().filter(|item| item.event.scope.project_id.as_deref() == Some(project_id)) {
            version += 1;
            ...
            match item.event.event_type.as_str() {
                ...
                "glossary.updated" => {
                    if let (Some(term), Some(value)) = (item.event.payload.get("term").and_then(Value::as_str), item.event.payload.get("value")) {
        ...
        Ok(ProjectContext {
            ...
            sync: SyncInfo { source: "memory_server".into(), last_sequence, stale: false },
        })
    }

    async fn ready(&self) -> anyhow::Result<()> { Ok(()) }
}
...
impl PostgresRepository {
    pub fn new(dsn: impl Into<Arc<str>>) -> Self { Self { dsn: dsn.into() } }

    async fn client(&self) -> anyhow::Result<tokio_postgres::Client> {
        let (client, connection) = tokio_postgres::connect(&self.dsn, NoTls).await.context("connect memory postgres")?;
...
impl MemoryRepository for PostgresRepository {
    async fn append(&self, event: MemoryEvent) -> anyhow::Result<AppendOutcome> {
        ...
        if let Some(row) = inserted {
            return Ok(AppendOutcome { event_id: event.event_id, status: AppendStatus::Accepted, sequence: row.get(0) });
        }
        let row = client.query_one("SELECT sequence FROM memory_events WHERE event_id = $1::text::uuid", &[&event.event_id]).await?;
        Ok(AppendOutcome { event_id: event.event_id, status: AppendStatus::Duplicate, sequence: row.get(0) })
    }

    async fn events_after(&self, sequence: i64, projects: &[String]) -> anyhow::Result<Vec<CommittedEvent>> {
        if projects.is_empty() { return Ok(Vec::new()); }
        let client = self.client().await?;
        ...
        rows.into_iter().map(|row| {
            ...
            Ok(CommittedEvent {
                sequence: row.get(0),
                event: MemoryEvent {
                    schema: EVENT_SCHEMA.into(), event_id: row.get(1), created_at: row.get(2),
                    scope: MemoryScope { project_id: row.get(3), task_id: row.get(4), session_id: row.get(5), thread_id: row.get(6) },
                    actor: MemoryActor { agent_id: row.get(7), agent_type: row.get(8), device_id: row.get(9) },
                    event_type: row.get(10),
                    memory: MemoryMeta { class: row.get(11), entity_type: row.get(14), entity_id: row.get(15), authority: u8::try_from(authority).map_err(|_| anyhow!("invalid stored authority"))?, confidence: row.get(13), expected_entity_version: None },
                    source: MemorySource { kind: row.get(18), r#ref: row.get(19), hash: row.get(20) },
                    supersedes: row.get(16), payload: row.get(17),
                },
            })
        }).collect()
    }
    ...
    async fn latest_sequence(&self) -> anyhow::Result<i64> {
        let client = self.client().await?;
        Ok(client.query_one("SELECT COALESCE(MAX(sequence), 0)::bigint FROM memory_events", &[]).await?.get(0))
    }
    ...
    async fn project_context(&self, project_id: &str) -> anyhow::Result<ProjectContext> {
        ...
        let Some(row) = row else {
            return Ok(ProjectContext { project_id: project_id.into(), version: 0, payload: Value::Null, sync: SyncInfo { source: "memory_server".into(), last_sequence: 0, stale: false } });
        };
        let last_sequence: i64 = row.get(7);
        Ok(ProjectContext {
            project_id: project_id.into(), version: row.get(0),
...
async fn health() -> Json<Value> { Json(json!({"status": "ok", "service": "zero3-memory-authority", "protocol": "v2.1"})) }

async fn ready(State(state): State<AppState>) -> Response {
    match state.repo.ready().await {
        Ok(()) => (StatusCode::OK, Json(json!({"status":"ready"}))).into_response(),
        Err(error) => (StatusCode::SERVICE_UNAVAILABLE, Json(json!({"status":"not_ready","error":error.to_string()}))).into_response(),
    }
}
    ...
    match state.repo.append(event.clone()).await {
        Ok(outcome) => {
            if outcome.status == AppendStatus::Accepted {
                let _ = state.bus.send(CommittedEvent { sequence: outcome.sequence, event });
            }
            (StatusCode::OK, Json(json!(outcome))).into_response()
        }
        Err(error) => (StatusCode::BAD_REQUEST, Json(json!({"error":"memory_event_rejected","message":error.to_string()}))).into_response(),
    }
}

#[derive(Deserialize)]
struct BatchRequest { events: Vec<MemoryEvent> }

async fn append_batch(State(state): State<AppState>, Json(batch): Json<BatchRequest>) -> Json<Value> {
    ...
    for event in batch.events {
        match state.repo.append(event.clone()).await {
            ...
            Err(error) => results.push(json!({"event_id":event.event_id,"status":"rejected","error":error.to_string()})),
        }
    }
    Json(json!({"results":results}))
}

async fn project_context(State(state): State<AppState>, Path(project_id): Path<String>) -> Response {
    match state.repo.project_context(&project_id).await {
        Ok(context) => (StatusCode::OK, Json(json!(context))).into_response(),
        Err(error) => (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error":"context_read_failed","message":error.to_string()}))).into_response(),
    }
}
...
async fn sync_socket(mut socket: WebSocket, state: AppState) {
    ...
    if hello.kind != "hello" || hello.protocol != SYNC_PROTOCOL || hello.client_id.is_empty() || hello.device_id.is_empty() || hello.last_sequence < 0 {
        let _ = socket.send(Message::Text(json!({"type":"error","code":"invalid_hello","message":"invalid sync hello"}).to_string())).await;
        return;
    }

    let latest = match state.repo.latest_sequence().await { Ok(value) => value, Err(_) => return };
    if socket.send(Message::Text(json!({"type":"ready","latest_sequence":latest}).to_string())).await.is_err() { return; }
    if let Ok(events) = state.repo.events_after(hello.last_sequence, &hello.projects).await {
        if !events.is_empty() {
            let from_sequence = events.first().map(|item| item.sequence).unwrap_or(hello.last_sequence);
            let to_sequence = events.last().map(|item| item.sequence).unwrap_or(hello.last_sequence);
...
mod tests {
    ...
    fn sample_event(id: &str, event_type: &str) -> MemoryEvent {
        MemoryEvent {
            schema: EVENT_SCHEMA.into(), event_id: id.into(), created_at: "2026-09-08T03:00:00Z".into(),
            scope: MemoryScope { project_id: Some("project-a".into()), task_id: None, session_id: None, thread_id: None },
            actor: MemoryActor { agent_id: "codex-1".into(), agent_type: "codex".into(), device_id: Some("desktop".into()) },
            event_type: event_type.into(),
            memory: MemoryMeta { class: "project".into(), entity_type: "decision".into(), entity_id: id.into(), authority: 60, confidence: Some(0.9), expected_entity_version: None },
            source: MemorySource { kind: "task".into(), r#ref: Some("task-1".into()), hash: None }, supersedes: Vec::new(), payload: json!({"text":"hello"}),
        }
    }
    ...
    async fn append_is_idempotent_and_context_is_projected() {
        ...
        for expected in ["accepted", "duplicate"] {
            let response = app.clone().oneshot(Request::post("/v1/memory/events").header("content-type", "application/json").body(Body::from(serde_json::to_vec(&event).unwrap())).unwrap()).await.unwrap();
            assert_eq!(response.status(), StatusCode::OK);
            let value: Value = serde_json::from_slice(&response.into_body().collect().await.unwrap().to_bytes()).unwrap();
            assert_eq!(value["status"], expected);
        }
        let response = app.oneshot(Request::get("/v1/projects/project-a/context").body(Body::empty()).unwrap()).await.unwrap();
        let value: Value = serde_json::from_slice(&response.into_body().collect().await.unwrap().to_bytes()).unwrap();
    ...
    async fn user_authority_boundary_is_fail_closed() {
        ...
        let response = app.oneshot(Request::post("/v1/memory/events").header("content-type", "application/json").body(Body::from(serde_json::to_vec(&event).unwrap())).unwrap()).await.unwrap();
